...
"""
    ...
    def __init__(
        ...
        self.port_1 = 19000
        self.port_2 = 29000
        self.replica_command_1 = f"clickhouse-server --config-file {self.config_file_replica_1} --daemon --pid-file {self.pid_file_replica_1} -- --path {self.run_path1} --user_files_path {self.user_files_path} --logger.stderr {self.log_dir}/stderr1.log --logger.log {self.log_dir}/clickhouse-server1.log --logger.errorlog {self.log_dir}/clickhouse-server1.err.log --tcp_port {self.port_1} --tcp_port_secure 19440 --http_port 18123 --https_port 18443 --interserver_http_port 19009 --tcp_with_proxy_port 19010 --mysql_port 19004 --postgresql_port 19005 --keeper_server.tcp_port 19181 --keeper_server.server_id 2 --prometheus.port 19988 --macros.replica r2"
        self.replica_command_2 = f"clickhouse-server --config-file {self.config_file_replica_2} --daemon --pid-file {self.pid_file_replica_2} -- --path {self.run_path2} --user_files_path {self.user_files_path} --logger.stderr {self.log_dir}/stderr2.log --logger.log {self.log_dir}/clickhouse-server2.log --logger.errorlog {self.log_dir}/clickhouse-server2.err.log --tcp_port {self.port_2} --tcp_port_secure 29440 --http_port 28123 --https_port 28443 --interserver_http_port 29009 --tcp_with_proxy_port 29010 --mysql_port 29004 --postgresql_port 29005 --keeper_server.tcp_port 29181 --keeper_server.server_id 3 --prometheus.port 29988 --macros.shard s2"
        self.proc = None
        self.proc_1 = None
        self.proc_2 = None
        self.pid = 0
...
"""
    ...
    def terminate(self):
        ...
        for proc, pid_file in (
            (self.proc, self.pid_file),
            (self.proc_2, self.pid_file_replica_2),
            (self.proc_1, self.pid_file_replica_1),
        ):
            if proc:
                # FIXME: Ends up with 'Application: Child process was terminated by signal 9 (KILL)' often (few jobs in one run)
                # if not Shell.check(
                #     f"clickhouse stop --pid-path {Path(pid_file).parent}", verbose=True
                # ):
    ...
    def prepare_logs(self, all=False):
        ...
        if all:
            ...
            if Path(self.MINIO_LOG).exists():
                res.append(Utils.compress_gz(self.MINIO_LOG))
            if Path(self.AZURITE_LOG).exists():
                res.append(Utils.compress_gz(self.AZURITE_LOG))
            if Path(self.GDB_LOG).exists():
                res.append(Utils.compress_gz(self.GDB_LOG))
            if Path(self.DMESG_LOG).exists():
                res.append(Utils.compress_gz(self.DMESG_LOG))
        self.logs = res
        return res
    ...
    def _collect_core_dumps(self):
        ...
        return [Utils.compress_gz(f) for f in core_files if Path(f).is_file()]
    ...
    def _get_logs_archives_server(self):
        ...
        return [Utils.compress_gz(self.log_dir)]
        ...
        for table in TABLES:
            ...
            if self.is_db_replicated:
                ...
                if not res:
                    ...
                    res = False
        return Utils.compress_gz(f"{temp_dir}/system_tables")
